...
const Withbutton = () => {
    ...
    const [button_number, setButtonNumber] = useState("");
    ...
    const handleDrop = (e, type) => { e.preventDefault(); addFile(e.dataTransfer.files[0], type); };
    const handleFileChange = (e, type) => { addFile(e.target.files[0], type); };
    ...
    const handleSubmit = async () => {
        ...
        formData.append("button_number", button_number);
        ...
        const campaignPromise = new Promise(async (resolve, reject) => {
            try {
                const token = localStorage.getItem("token");
                const response = await fetch(`${import.meta.env.VITE_API_URL}/api/auth/createcampaign_withbutton`, {
                    method: "POST",
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
            ...
            <div className="flex-1 flex flex-col">
                ...
                <div className="flex justify-between items-center py-8 px-8">
                    <h2 className="text-2xl font-bold text-gray-800">
                        Send With Button WhatsApp Message
                    </h2>
                    {user && (
                    ...
                    <div className="bg-white p-6 rounded-lg shadow-xl">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                                    ...
                                    >
                                        ...
                                        {csvFile && (
                                            <div className="mt-2 flex justify-center items-center text-green-600 space-x-2">
                                                ...
                                                <button onClick={() => removeFile('csv')} className="text-red-600 hover:text-red-800 font-bold">×</button>
                                            </div>
                                        )}
                                    </div>
                                    ...
                                    >
                                        ...
                                        {excelFile && (
                                            <div className="mt-2 flex justify-center items-center text-green-600 space-x-2">
                                                ...
                                                <button onClick={() => removeFile('excel')} className="text-red-600 hover:text-red-800 font-bold">×</button>
                                            </div>
                                        )}
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    >
                                        <div className="flex justify-between items-center font-semibold text-gray-700">
                                            FILE FORMAT GUIDELINES
                                            <span className={`text-sm transform transition-transform duration-300 ${openFileGuidelines ? "rotate-180" : ""}`}>▼</span>
                                        </div>
                                    </div>
                                    <div className={`overflow-hidden transition-all duration-500 ease-in-out transform ${openFileGuidelines ? "max-h-96 opacity-100 scale-y-100" : "max-h-0 opacity-0 scale-y-95"} origin-top`}>
                            ...
                            <div className="space-y-6">
                                ...
                                {/* Call To Action Form */}
                                <div className="bg-white p-6 rounded-lg shadow-md max-w-xl border border-gray-300">
                                    <div className="flex items-center border border-gray-300 rounded-lg overflow-hidden mb-4">
                                        <label htmlFor="title" className="w-1/3 px-4 py-3 text-sm font-medium text-gray-700 bg-gray-100 border-r border-gray-300">Title</label>
                                        <input type="text" id="title" placeholder="Visit Now / Call Now" className="w-2/3 px-4 py-3 text-sm text-black placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500" value={button_title} onChange={(e) => setButtonTitle(e.target.value)} />
                                    </div>
                                    <div className="flex items-center border border-gray-300 rounded-lg overflow-hidden mb-4">
                                        <label htmlFor="url" className="w-1/3 px-4 py-3 text-sm font-medium text-gray-700 bg-gray-100 border-r border-gray-300">Url / Link</label>
                                        <input type="url" id="url" placeholder="http://" className="w-2/3 px-4 py-3 text-sm text-black placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500" value={button_url} onChange={(e) => setButtonUrl(e.target.value)} />
                                    </div>
                                    <div className="flex items-center border border-gray-300 rounded-lg overflow-hidden">
                                        <label htmlFor="number" className="w-1/3 px-4 py-3 text-sm font-medium text-gray-700 bg-gray-100 border-r border-gray-300">Number</label>
                                        <input type="digit" maxLength={10} id="number" placeholder="Enter Number" className="w-2/3 px-4 py-3 text-sm text-black placeholder-gray-400 focus:outline-none focus:ring- focus:ring-blue-500" value={button_number} onChange={(e) => setButtonNumber(e.target.value)} />
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    >
                                        <div className="flex justify-between items-center font-semibold text-gray-700">
                                            UPLOAD GUIDELINES
                                            <span className={`text-sm transform transition-transform duration-300 ${openUploadGuidelines ? "rotate-180" : ""}`}>▼</span>
                                        </div>
                                    </div>
                                    <div className={`overflow-hidden transition-all duration-500 ease-in-out transform ${openUploadGuidelines ? "max-h-96 opacity-100 scale-y-100" : "max-h-0 opacity-0 scale-y-95"} origin-top`}>
